...
var nodecache = function(options) {
  ...
  self.cleanarray = function(cleaningratio) {
    ...
    for (var i = 0; i < numberOfElements; i++) {
      ...
      if(element && element.ranking <= self.status.average) {
        var oldElementSize = self.getsize(self.array[allKeys[i]], allKeys[i]);
        self.status.memorysize = self.status.memorysize - oldElementSize;

        var ranking = self.array[allKeys[i]].ranking;
        self.status.average = parseInt( ( (self.status.average * self.status.count  ) 
        - (ranking) )  / self.status.count);
  ...
  self.put = function(ranking, key, value) {
    ...
    if(typeof(key) != "undefined" &&
      ...
      if(self.status.memorysize + elementsize  >= self.options.maxsize) {
        ...
        if(self.status.count <= 10) {
          // 100% cleaning ratio - edge case
          self.cleanarray(10000);
        } else {
          // 50% cleaning ratio
      ...
      self.status.average = parseInt( ( (self.status.average * (self.status.count - 1) ) 
        + (ranking) )  / self.status.count);
